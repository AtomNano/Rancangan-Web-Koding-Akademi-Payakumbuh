    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('cs.siswa.update', $user->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div class="mt-1 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <p class="text-xs text-gray-500 mt-1">Format: NNN-KODEKELASNUM2-MMYYYY (contoh:
                                        001-01-122025)</p>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="mt-1 relative">
                                    ...
                                    <x-text-input id="no_telepon" class="block w-full pl-12" type="tel"
                                        name="no_telepon" :value="old('no_telepon', $user->no_telepon)"
                                        placeholder="81234567890" pattern="[0-9]{10,13}" />
                                </div>
                            ...
                            <div>
                                ...
                                <x-text-input id="tanggal_lahir" class="block mt-1 w-full" type="date"
                                    name="tanggal_lahir" :value="old('tanggal_lahir', $user->tanggal_lahir ? $user->tanggal_lahir->format('Y-m-d') : '')"
                                ...
                                <div class="mt-2 space-y-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                    ...
                                    <div>
                                        ...
                                        <x-text-input id="jalan" class="block mt-1 w-full" type="text" name="jalan"
                                            :value="old('jalan', $alamatParts['jalan'] ?? '')"
                    ...
                    <div class="p-6 bg-gray-50 border-b border-gray-200">
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="tanggal_pendaftaran" class="block mt-1 w-full" type="date"
                                    name="tanggal_pendaftaran" :value="old('tanggal_pendaftaran', $user->tanggal_pendaftaran ? $user->tanggal_pendaftaran->format('Y-m-d') : date('Y-m-d'))" />
                            </div>
                            @php
                            ...
                            <div>
                                ...
                                <x-text-input id="sekolah" class="block mt-1 w-full" type="text" name="sekolah"
                                    :value="old('sekolah', $sekolahValue)" placeholder="Contoh: SD Negeri 01 Payakumbuh"
                                    required />
                                    ...
                                    @foreach ($kelas as $item)
                                        <label class="flex items-center">
                                            <input type="checkbox" name="bidang_ajar[]" value="{{ $item->nama_kelas }}"
                                                ...
                                                {{ in_array($item->nama_kelas, $selectedBidangAjar) ? 'checked' : '' }}>
                                            <span class="ml-2 text-sm text-gray-600">{{ $item->nama_kelas }}</span>
                                        </label>
                                    @endforeach
                                </div>
                                <p class="mt-2 text-sm text-gray-500">Pilih satu atau lebih kelas untuk siswa. Semua
                                    jenis kelas tersedia (Dasar, Umum, Mahasiswa, dll).</p>
                            </div>
                            <div>
                                <x-input-label for="durasi" :value="__('Durasi Program')" />
                                <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <label
                                        class="flex items-center p-3 border-2 rounded-lg cursor-pointer hover:bg-indigo-50 transition-colors {{ old('durasi', $user->durasi) == '1 Bulan' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="radio" name="durasi" value="1 Bulan"
                                            class="text-indigo-600 focus:ring-indigo-500" {{ old('durasi', $user->durasi) == '1 Bulan' ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm font-medium text-gray-700">1 Bulan</span>
                                    </label>
                                    <label
                                        class="flex items-center p-3 border-2 rounded-lg cursor-pointer hover:bg-indigo-50 transition-colors {{ old('durasi', $user->durasi) == '3 Bulan' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="radio" name="durasi" value="3 Bulan"
                                            class="text-indigo-600 focus:ring-indigo-500" {{ old('durasi', $user->durasi) == '3 Bulan' ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm font-medium text-gray-700">3 Bulan</span>
                                    </label>
                                    <label
                                        class="flex items-center p-3 border-2 rounded-lg cursor-pointer hover:bg-indigo-50 transition-colors {{ old('durasi', $user->durasi) == '6 Bulan' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="radio" name="durasi" value="6 Bulan"
                                            class="text-indigo-600 focus:ring-indigo-500" {{ old('durasi', $user->durasi) == '6 Bulan' ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm font-medium text-gray-700">6 Bulan</span>
                                    </label>
                                    <label
                                        class="flex items-center p-3 border-2 rounded-lg cursor-pointer hover:bg-indigo-50 transition-colors {{ old('durasi', $user->durasi) == '12 Bulan' ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200' }}">
                                        <input type="radio" name="durasi" value="12 Bulan"
                                            class="text-indigo-600 focus:ring-indigo-500" {{ old('durasi', $user->durasi) == '12 Bulan' ? 'checked' : '' }}>
                            ...
                            @endphp
                            <div
                                class="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4 p-4 rounded-lg border border-gray-200 bg-white">
                                <div>
                                    ...
                                    <x-text-input id="enrollment_start_date" name="enrollment_start_date" type="date"
                                        class="block mt-1 w-full" value="{{ $enStart ?: date('Y-m-d') }}" required />
                                    <p class="text-xs text-gray-500 mt-1">Tanggal sesi pertama atau tanggal bergabung.
                                    </p>
                                </div>
                                <div>
                                    ...
                                    <select id="enrollment_monthly_quota" name="enrollment_monthly_quota"
                                        ...
                                        required>
                                        @foreach([4, 8] as $q)
                                            <option value="{{ $q }}" {{ (int) $enQuota === $q ? 'selected' : '' }}>{{ $q }}x
                                                per bulan</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="md:col-span-3">
                                    <div
                                        class="flex items-center justify-between bg-indigo-50 text-indigo-800 px-4 py-2 rounded">
                                        <span class="text-sm font-medium">Target total sesi</span>
                                        <span class="text-lg font-semibold"
                                            id="target_sessions_display">{{ $enTarget ? $enTarget . ' sesi' : '-' }}</span>
                                    </div>
                                    <input type="hidden" name="enrollment_duration_months"
                                        id="enrollment_duration_months"
                                        value="{{ old('enrollment_duration_months', $enDuration) }}">
                                    <input type="hidden" name="enrollment_target_sessions"
                                        id="enrollment_target_sessions"
                                        value="{{ old('enrollment_target_sessions', $enTarget) }}">
                                    <p class="text-xs text-gray-500 mt-1">Akan menonaktifkan akun setelah target sesi
                                        tercapai.</p>
                                </div>
                            </div>
                            <div class="md:col-span-2">
                                    ...
                                    @foreach(['Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu', 'Minggu'] as $day)
                                        <label class="flex items-center">
                                            <input type="checkbox" name="hari_belajar[]" value="{{ $day }}"
                                                class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500"
                                                {{ is_array($old_hari_belajar) && in_array($day, $old_hari_belajar) ? 'checked' : '' }}>
                                ...
                                <div class="mt-2 flex space-x-4">
                                    <label class="flex items-center">
                                        <input type="radio" name="enrollment_status" value="active"
                                            class="text-indigo-600 focus:ring-indigo-500" {{ $currentEnrollmentStatus == 'active' ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm text-gray-600">Aktif</span>
                                    </label>
                                    <label class="flex items-center">
                                        <input type="radio" name="enrollment_status" value="inactive"
                                            class="text-indigo-600 focus:ring-indigo-500" {{ $currentEnrollmentStatus == 'inactive' ? 'checked' : '' }}>
                                        <span class="ml-2 text-sm text-gray-600">Tidak Aktif</span>
                                    </label>
                                </div>
                                <p class="mt-2 text-sm text-gray-500">Status akan otomatis menjadi tidak aktif jika
                                    durasi program telah berakhir.</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select id="metode_pembayaran" name="metode_pembayaran"
                                    ...
                                    <option value="transfer" {{ old('metode_pembayaran', $user->metode_pembayaran) == 'transfer' ? 'selected' : '' }}>Transfer Bank
                                    </option>
                            ...
                            <div>
                                ...
                                <select id="status_promo" name="status_promo"
                                    ...
                                    <option value="Promo Saudara Gratis" {{ old('status_promo', $user->status_promo) == 'Promo Saudara Gratis' ? 'selected' : '' }}>Promo Saudara
                                        Gratis</option>
                                ...
                                <p class="mt-1 text-xs text-gray-500">Pilih promo atau diskon yang berlaku (jika ada)
                                </p>
                            </div>
                            <div>
                                <x-input-label for="biaya_pendaftaran" :value="__('Biaya Pendaftaran')" />
                                <x-text-input id="biaya_pendaftaran" class="block mt-1 w-full" type="text"
                                    name="biaya_pendaftaran" :value="old('biaya_pendaftaran', $user->biaya_pendaftaran ? 'Rp. ' . number_format($user->biaya_pendaftaran, 0, ',', '.') : 'Rp. 150.000')" />
                            </div>
                            <div>
                                <x-input-label for="biaya_angsuran" :value="__('Biaya Angsuran')" />
                                <x-text-input id="biaya_angsuran" class="block mt-1 w-full" type="text"
                                    name="biaya_angsuran" :value="old('biaya_angsuran', $user->biaya_angsuran ? 'Rp. ' . number_format($user->biaya_angsuran, 0, ',', '.') : 'Rp. 1.250.000')" />
                            </div>
                            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <x-input-label for="total_biaya" :value="__('Total Biaya')" />
                                    <x-text-input id="total_biaya" class="block mt-1 w-full bg-gray-200" type="text"
                                        name="total_biaya" :value="old('total_biaya', $user->total_biaya ? 'Rp. ' . number_format($user->total_biaya, 0, ',', '.') : '')" readonly />
                                </div>
                                <div>
                                    ...
                                    <select id="discount_type" name="discount_type"
                                        ...
                                        <option value="percentage" {{ old('discount_type', $user->discount_type) == 'percentage' ? 'selected' : '' }}>Persentase (%)
                                        </option>
                            ...
                            <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <x-text-input id="discount_value" class="block mt-1 w-full" type="text"
                                        name="discount_value" :value="old('discount_value', $user->discount_value)" />
                                </div>
                                <div>
                                    <x-input-label for="total_setelah_diskon" :value="__('Total Setelah Diskon')" />
                                    <x-text-input id="total_setelah_diskon" class="block mt-1 w-full bg-gray-200"
                                        type="text" name="total_setelah_diskon" :value="old('total_setelah_diskon', $user->total_setelah_diskon ? 'Rp. ' . number_format($user->total_setelah_diskon, 0, ',', '.') : '')" readonly />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end p-6 bg-gray-50">
                        <a href="{{ route('cs.siswa.index') }}" class="text-sm text-gray-600 hover:text-gray-900 mr-4">
                            {{ __('Batal') }}
                        </a>
                        <x-primary-button>
                            {{ __('Simpan Perubahan') }}
