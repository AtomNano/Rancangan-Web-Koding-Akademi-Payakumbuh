...
<div x-show="sidebarOpen" @click="sidebarOpen = false" x-transition:enter="transition-opacity ease-linear duration-300"
    ...
    x-transition:leave-end="opacity-0" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40 md:hidden" x-cloak
    style="display: none;"></div>

<!-- Sidebar -->
<div @mouseenter="sidebarOpen = true" @mouseleave="if(window.innerWidth >= 768) sidebarOpen = false"
    @click="sidebarOpen = true" :class="{
    ...
    <div class="flex flex-col h-full">
        <!-- Sidebar Header -->
        <div
            class="flex items-center h-20 px-2 border-b border-slate-700/50 flex-shrink-0 bg-slate-900/50 backdrop-blur-sm overflow-hidden">
            <a href="{{ route('dashboard') }}"
                class="flex items-center w-full group transition-all duration-200 min-w-0"
                ...
                <div x-show="sidebarOpen" x-transition:enter="transition ease-out duration-200"
                    ...
                    x-transition:leave-end="opacity-0 transform scale-95" class="flex flex-col ml-3 min-w-0 flex-1">
        ...
        <nav id="sidebar-nav"
            ...
            @if($user->role === 'admin')
                ...
                <div class="mt-8">
                    <h3 x-show="sidebarOpen" class="px-3 mb-3 text-xs font-semibold text-slate-400 uppercase tracking-wider"
                        x-cloak>Manajemen</h3>
                    <div class="space-y-1.5">
                        ...
                        <x-nav-link :href="route('admin.pertemuan.select')"
                            :active="request()->routeIs('admin.pertemuan.*')"
                            title="Pilih kelas terlebih dahulu, lalu kelola pertemuan & absen">
                            <svg class="h-5 w-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <x-nav-link :href="route('admin.backup.index')" :active="request()->routeIs('admin.backup.*')">
                            <svg class="h-5 w-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M4 7v10c0 2.21 3.582 4 8 4s8-1.79 8-4V7M4 7c0-2.21 3.582-4 8-4s8 1.79 8 4M4 7s0 4 8 4 8-4 8-4" />
                            </svg>
            ...
            @elseif($user->role === 'cs')
                ...
                <div class="space-y-1.5">
                    <x-nav-link :href="route('cs.dashboard')" :active="request()->routeIs('cs.dashboard')">
                        <svg class="h-5 w-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z" />
                    ...
                    <x-nav-link :href="route('cs.siswa.index')" :active="request()->routeIs('cs.siswa.*')">
                        <svg class="h-5 w-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m-4.5 3.903A2.5 2.5 0 017 15h10a2.5 2.5 0 012.5 2.5V21" />
                        </svg>
                        <span x-show="sidebarOpen" class="ml-3 whitespace-nowrap" x-cloak>Data Siswa</span>
                    </x-nav-link>
                    <x-nav-link :href="route('cs.logs')" :active="request()->routeIs('cs.logs')">
                        <svg class="h-5 w-5 flex-shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <span x-show="sidebarOpen" class="ml-3 whitespace-nowrap" x-cloak>Log Aktivitas</span>
                    </x-nav-link>
                </div>
            ...
            <div class="flex items-center">
                <div class="flex-shrink-0">
                    <img class="h-10 w-10 rounded-lg object-cover ring-2 ring-slate-700/50"
                        src="{{ $user->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&color=FFFFFF&background=475569' }}"
                        alt="{{ $user->name }}">
                </div>
